...
/* Parse the command line according to option descriptions in
   OPTS.  */
int
ulib_options_parse(const ulib_option *opts, int argc, const char *argv[], FILE *err) {
    ...
    while (next_arg < argc) {
        ...
        if (opt->key == 0 && opt->name == 0) {
            ...
            if (argv[next_arg][0] == '-') {
                ...
                fprintf(err,
                        "%s: WARNING: Non-option argument ``%s''"
                        " not processed\n",
                        argv[0],
                        argv[next_arg]);
            ...
            if (opt->flags == ulib_option_no_arg) {
                ...
                if (opt_arg == 0) {
                    /* If the next argument does not begin with a dash,
                     ...
                     option.  */
                    if (!next_arg_taken && next_arg + 1 < argc
                        && argv[next_arg + 1][0] != '-') {
                        ...
                        if (key == -1)
                            fprintf(err,
                                    "%s: ERROR: Option ``--%s'' requires"
                                    " an argument\n",
                                    argv[0],
                                    opt->name);
                        else
                            fprintf(err,
                                    "%s: ERROR: Option ``-%c'' requires"
                                    " an argument\n",
                                    argv[0],
                                    opt->key);
            ...
            if (opt->cb == 0 && opt_arg != 0) {
                fprintf(err,
                        "%s: ERROR: Unable to process option"
                        " argument ``%s''\n",
                        argv[0],
                        opt_arg);
                return -1;

...
int
ulib_hash_init(ulib_hash *table,
               unsigned int length,
               ulib_hash_func hash,
...
ulib_hash_lookup(const ulib_hash *table, const ulib_list *key) {
    ...
    for (lst = table->dir[idx].next; lst != (void *)(table->dir + idx); lst = lst->next)
...
ulib_hash_traverse(ulib_hash *table, ulib_visit_func visit, void *data) {
    ...
    i = table->length;
    while (i--)
        for (elt = table->dir[i].next; elt != (void *)&table->dir[i]; elt = elt->next)
            if (visit(elt, data))
                break;
}
